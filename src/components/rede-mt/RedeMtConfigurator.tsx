...
export function RedeMtConfigurator({ propostaId, criadoPor }: { propostaId?: string; criadoPor?: string }) {
  ...
  const equipeOrc = useEquipeResponsavel({ servicoKey: "rede-mt", criadoPor, escopo: "orcamento" });
  // As duas frentes somadas entram no custo do PROJETO, antes do Fator K.
  ...
  useEffect(() => {
    if (propostaId) {
      fetch(`/api/propostas/${propostaId}`).then((r) => r.json()).then((d) => {
        if (d.proposta?.dados) { const dados = d.proposta.dados as Partial<Form> & { cond?: CondPag; custoRows?: CustoRow[]; equipeGta?: unknown; equipeOrcamento?: unknown }; setForm({ ...FORM_INICIAL, ...dados }); precoTocado.current = true; if (dados.equipeGta) equipe.restaurar(dados.equipeGta as EquipeSalva); if (dados.equipeOrcamento) equipeOrc.restaurar(dados.equipeOrcamento as EquipeSalva);  if (dados.cond) setCond(dados.cond as CondPag); if (Array.isArray(dados.custoRows) && dados.custoRows.length) setCustoRows(dados.custoRows); }
      }).catch(() => {});
    } else {
  ...
  const calcKey = JSON.stringify([custoProjeto, custoExecucao, custoEquipeTotal, recalcNonce]);
    ...
    debounce.current = setTimeout(async () => {
      try {
        const res = await fetch("/api/rede-mt/calcular", {
          ...
          body: JSON.stringify({ custoProjeto, custoExecucao, custoEquipe: custoEquipeTotal }),
        });
        if (res.ok) {
  ...
  async function salvar(silencioso = false) {
    ...
    try {
      const payload = { serviceKey: "rede-mt", cliente: form.clienteNome, status: totalCliente > 0 ? "precificada" : "rascunho", dados: { ...form, cond, custoRows, equipeGta: equipe.serializar(), equipeOrcamento: equipeOrc.serializar() } };
      const res = savedId
    ...
    <div className="space-y-6">
      ...
      <section className="section-card">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
          <button type="button" className="btn-link" onClick={addRow}>+ Adicionar item de custo</button>
      ...
      <section className="section-card">
        <div className="flex items-center justify-between">
          ...
          {preco && precoTocado.current && preco.faturamentoTotal > 0 && (
            <button type="button" className="btn-link text-xs" onClick={() => { precoTocado.current = false; setForm((f) => ({ ...f, valorProjeto: nf(preco.faturamentoProjeto, 2), valorExecucao: nf(preco.faturamentoExecucao, 2) })); }}>Usar sugerido {brl(preco.faturamentoTotal)}</button>
          )}
        </div>
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-6">
